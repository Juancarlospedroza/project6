...
if opcion == "De 20,000 a menos de 30,000 DLS":
    st.dataframe(df[(df["price"] >= 20000) & (df["price"] < 30000)].sort_values(by="price").reset_index(drop=True))

if opcion == "De 30,000 a menos de 40,000 DLS":
    st.dataframe(df[(df["price"] >= 30000) & (df["price"] < 40000)].sort_values(by="price").reset_index(drop=True))
